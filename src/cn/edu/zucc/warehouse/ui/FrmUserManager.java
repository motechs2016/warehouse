...
public class FrmUserManager extends JDialog implements ActionListener {
	...
	private Button btnResetStatus = new Button("设置用户状态");
	private Object tblTitle[]={"账号","姓名","类别","状态"};
	...
	private void reloadUserTable(){
		try {
			...
			for(int i=0;i<users.size();i++){
				...
				tblData[i][3]=users.get(i).getStatus();
			}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==this.btnAdd){
			...
			if(JOptionPane.showConfirmDialog(this,"确定删除账号吗？","确认",JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION){
				...
				if(userid.equals(SystemUserManager.currentUser.getUser_id()))
				{
					JOptionPane.showMessageDialog(null,  "不能删除当前登录的账号","提示",JOptionPane.ERROR_MESSAGE);
					return;
				}  
			...
			int i=this.userTable.getSelectedRow();
			String userid=this.tblData[i][0].toString();
			FrmUserManager_ResetStatus dlg=new FrmUserManager_ResetStatus(this,"设置账号状态",true,userid);
			dlg.setVisible(true);
			if(dlg.getUser()!=null){//刷新表格
				this.reloadUserTable();
			}
